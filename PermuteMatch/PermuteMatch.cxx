...
namespace csl {
    ...
    PermuteMatch::PermuteMatch(Alphabet alph_file, char* db_file,char* dic_file,int dlev) : alph(alph_file), levquery(alph,dic_file, dlev), db(alph,db_file), list_(), findParts(false) ,minParts(1) {
...
//	std::cout<<"rek_query(db_pos="<<db_pos<<",w_pos="<<w_pos<<",col_bits="<<col_bits<<",depth="<<depth<<")"<<std::endl;
	...
	if(db.isFinal(db_pos) && // db_pos must be a final state AND
	   ((findParts && depth>=minParts) || // either the findParts-switch must be active with at least minParts tokens being processed OR .. 
	    ...
	    TransTable::AnnIterator it(db,db_pos);
	    for(;it.isValid();++it) {
...
//	       (std::cout<<"col still open, "<<std::flush) &&
	       ...
	       (new_db_pos = db.walk(new_db_pos,alph.code(Global::Perm::tokenDelimiter))) && // could walk delimiter in db
