...
namespace csl {
    ...
    PermuteMatch::PermuteMatch(Alphabet alph_file, char* db_file,char* dic_file,int dlev) : alph_(alph_file), levquery_(alph_,dic_file, dlev), db_(alph_,db_file), list_(), findParts(true) ,minParts(1) {
...
//	    printf("Try with: %s\n",(char*)list_.at(listPos).getStr());
	    ...
	    if(!findParts) { // findParts deactivated
...
//		    std::cout<<"could traverse in db complete token: "<<list_.at(listPos).getStr()<<std::endl;
		...
		if((newNewDbPos = db_.walk(newDbPos,compDelim)) &&
		   (findParts || (newColBits == b11[depth]))) {  // if not findParts, used exactly a prefix of the query string
