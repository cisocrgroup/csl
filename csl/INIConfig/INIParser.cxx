...
double iniparser_getdouble(dictionary * d, const char * key, double notfound)
{
    ...
    std::locale loc( CSL_UTF8_LOCALE, new std::numpunct< char >() );
    sstream.imbue( loc );
...
dictionary * iniparser_load(const char * ininame)
{
    ...
    std::ifstream in;
    // utf8 locale. make sure punctuation is in basic style, i.e., a '.' for decimals
    in.imbue( std::locale( CSL_UTF8_LOCALE, new std::numpunct< wchar_t >() ) );
    ...
    while( in.good() ) {
	in.getline( line+last, ASCIILINESZ-last );
        lineno++ ;
        len = (int)strlen(line);
