...
void read_dict (tdict * dict, FILE * fi, int size) {
    ...
    dict->strings = (uchar *) & dict->cells[dict->cells[1].tc.b]; // ???
}
...
void find_intersection3(int d, int cs, int i, int j) {
    ...
    if (dict->cells[d].tc.c==0 && chv->n-i<2*lvt->k+1 && lvt->fin[f_ind(*lvt,2*lvt->k-(chv->n-i),cs)]>distance2) {

	if (rev) {
	    for (t=strlen(s)-1, c1=0; t>=0; t--, c1++) sr[c1]=s[t]; sr[c1]=0;
...
void find_intersection1(int d, int cs, int i, int j) {
    ...
    if (chv_->n-i<2*lvt_->k+1 && lvt_->fin[f_ind(*lvt_,2*lvt_->k-(chv_->n-i),cs)]>distance1) {
