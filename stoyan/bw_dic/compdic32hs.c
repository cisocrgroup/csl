...
/**
 * Creates a hashcode for a state
 * @param i an index into the array of temporary states
 * @param k k is the (alphabetically) last char that labels a transition from state i
 */
...
void hashresize() {
    int * sht1; // new version of states_hash_table
    int * hc1; // new version of hash_codes
    int i, 
	hashs1, // new version of HASHS (hashtable size)
	p, 
	h;
